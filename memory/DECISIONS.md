...
## 2026-09-05 · GitHub 上传授权（私有策略已被后续公开要求替代）
用户先明确要求上传整个插件工程，并确认目标账号 Liiiin-hku；替代初始“不建远程、不推送”的限制。首次采用私有仓库，源码、锁文件、虚构测试和脱敏开发记忆进入 Git，可安装插件包放 Release。该私有策略仅保留为历史原因，当前按下一条公开策略维护。个人资料、凭据、依赖缓存和测试浏览器目录不上传的边界继续有效。

## 2026-09-05 · 公开读取与朋友试用
用户明确要求将同一仓库公开，让其他人读取、下载和使用，同时不赋予修改原仓库的权限；已通过官方 API 设置 public。公开访客不需要成为 Collaborator，权限核验只有所有者 admin，无其他协作者、待接受邀请或部署密钥。保留 Issue 反馈和 Fork / PR 入口，由所有者决定是否合并；不能把禁止直接写原仓库描述成禁止修改他人本地副本。

安装仍使用已校验的 v0.1.0 Release，标签和附件不覆盖。README 与 docs/朋友试用.md 提供明确的安装使用授权和操作步骤；不擅自引入额外的商业 / 衍生作品开源许可。package.json 的 private 字段仍防止 npm 意外发布，与仓库公开无关。发布脚本的预期可见性改为 public；以后再次改变可见性或授予其他账号写权限须有明确用户要求。
